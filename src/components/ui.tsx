...
export function Card({ children, className = '' }: { children: ReactNode; className?: string }) {
  return (
    <div className={`bg-surface border border-line rounded-2xl p-4 sm:p-6 shadow-[var(--shadow-card)] ${className}`}>
      {children}
    </div>
  );
}
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-[2px] flex items-center justify-center z-50 px-4">
      <div className="bg-surface border border-line rounded-2xl w-full max-w-md p-6 shadow-[var(--shadow-card)] max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-5">
          ...
          <button onClick={onClose} className="text-muted hover:text-ink transition-colors" aria-label="Fechar">
            <X size={20} />
          </button>
        </div>
        {children}
